...
//map是表示的数据hashmap可以用成对的name/value来保存和取出
...
// java中遍历map的方法Set<String> set = map.keySet();   
 // for (String s:set) {  
 ...
 mFreqMatrix.get(mAllItems[i]));
	...
	private void printMatrixes(Map<ItemId,Float> ratings,
	    	...
	    	for (int j=0; j<mAllItems.length; j++) {
	    		System.out.format("%10.3f", ratings.get(mAllItems[j]));//format是格式化
	    		System.out.print(" ");
